...
def edit_album(request, album_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save(commit=True)
            diction.update({'success_text':'Data Successfully Updated'})
...
def delete_album(request, album_id):
    ...
    diction = {"delete_success":"Album Successfully Deleted !"}
...
def delete_musician(request, artist_id):
    ...
    diction = {"delete_success":"Musician Successfully Deleted !"}
    return render(request, "first_app/delete.html", context=diction)
